...
import razorpay
from foodDelivery_system.settings import RZP_KEY_ID, RZP_KEY_SECRET
from django.contrib.sites.shortcuts import get_current_site


#config rozer pay as per documentation
...
# below code is for when we add orders from different vender so orderd-venders need to preselected form all venders filed  in admin panal
    ...
    subtotal = get_cart_amounts(request)['subtotal'] #we are getting this tax from marketplace context processor
    ...
    if request.method == 'POST':        #if request of form is post then user had submited the form 
        ...
        if form.is_valid():
            ...
            # RazorPay Payment(create razorpay server predefine code by rozerpay)
            DATA = {
...
def payments(request):
    ...
    if request.headers.get('x-requested-with') == 'XMLHttpRequest' and request.method == 'POST':
        ...
        # SEND ORDER CONFIRMATION EMAIL TO THE CUSTOMER(we take send_notification function form utils.py which is already create to handle notification )
